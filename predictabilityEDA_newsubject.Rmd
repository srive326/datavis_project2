...
count <- 0
for (k in :length(listy)){
  
hello <- myFunction(listy[[k]])

list_forlinegraph[[k]] <- hello
...
for (s in 1:length(list_forlinegraph)){
p<-ggplot(list_forlinegraph[[s]], aes(x=factor(day, level = c("Sunday","Monday","Tuesday","Wednesday","Thursday", "Friday","Saturday")), y=Hour_Home, group=week)) +
  geom_line(aes(color=week))+
  geom_point(aes(color=week)) + 
  labs(title = "Subjects' Hour Home from Work") +
  labs(x = "Day of Week") +
  labs(y = "Hour (Out of 24)") 

list_ofplots[[s]] <- p
}

vec_ofsubjects <- c(3,4,7,8,9, 10,13,15,16,17,19,20,21,22,23,25,26,27,29,33,34,35,36,37,38,40,41,42,49,50,52,53,54,55,56,60,61,62,63,67,70,72,73,74,75,76,77,78,79,81,82,83,86,88,89,90,93,94,96,97,98,99,101,103)

length(vec_ofsubjects)

vec_ofsubjects[c(16,39,63)]

subjects_linegraph <- as.matrix(vec_ofsubjects[!vec_ofsubjects %in% vec_ofsubjects[c(16,39,63)]])

names(subjects_linegraph)[1] <- c("Subject Number")
